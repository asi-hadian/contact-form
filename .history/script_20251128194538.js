...
radios.forEach(radio => {
  radio.addEventListener("change", () => {
    document.querySelectorAll(".radio-group").forEach(g => g.classList.remove("checked"));
...
if(!document.querySelector('input[name=query_type]:checked')){
    document.getElementById('radio-error').textContent='please select a query type'
   valid=false
}
...
if(!consent.checked){
    document.querySelector('#consent-error').textContent='to submitthis form please consent to being contacted'
    valid=false
}
